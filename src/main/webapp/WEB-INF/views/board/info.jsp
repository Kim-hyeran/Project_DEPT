       ...
       <c:choose>
			...
			<c:otherwise>
				<c:forEach items="${INFO_LIST}" var="INFO_VO" varStatus="i">
			        <tr id="tr_body">
			            <td>${i.count}</td>
			            <td class="board_title" data-seq="${INFO_VO.cs_info_seq}"><a href="${rootPath}/comsc/info/${INFO_VO.cs_info_seq}">${INFO_VO.cs_info_title}</a></td>
			            <td>${INFO_VO.cs_info_writer}</td>
			            <td>${INFO_VO.cs_info_date}</td>
			            <td>${INFO_VO.cs_info_count}</td>
			            <td></td>
			        </tr>
				</c:forEach>
			</c:otherwise>
